...
const WEBHOOK_URL = getEnv(
  'WEBHOOK_URL',
  'https://api.demo.wonderful.ai/api/v1/tasks/webhook/7a86a4e3-ed34-4307-b44c-57bc94f795b3'
);
const WEBHOOK_SECRET = getEnv(
  'WEBHOOK_SECRET',
  '05d5dc71-c710-470d-88bf-6f038763b95b'
);
...
const triggerWebhook = async ({ email, firstName, lastName, attachmentId }) => {
  const subject = `בקשת ביטוח - ${firstName} ${lastName}`.trim();
  ...
  const webhookResponse = await fetch(WEBHOOK_URL, {
    ...
    body: JSON.stringify({
      ...
      payload: {
        ...
        subject,
        message: 'Process this insurance request',
