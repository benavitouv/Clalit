...
const WEBHOOK_URL = getEnv(
  'WEBHOOK_URL',
  'https://api.demo.wonderful.ai/api/v1/tasks/webhook/390083e8-4ee7-486d-9cd0-a271d19faf62'
);
const WEBHOOK_SECRET = getEnv(
  'WEBHOOK_SECRET',
  '4e820097-9184-4deb-bc3e-15a8f0b43db0'
);
...
const triggerWebhook = async ({ email, firstName, lastName, hospital, attachmentId }) => {
  const subject = `בקשה לטופס 17 - ${firstName} ${lastName}`.trim();
    ...
    try {
      ...
      if (!file || typeof file === 'string') {
        jsonResponse(res, 400, {
          ok: false,
          error: 'missing_file',
          message: 'חובה לצרף הפניה רפואית.',
        });
        return;
      }
...
server.listen(PORT, () => {
  console.log(`Clalit Form 17 UI server running at http://localhost:${PORT}`);
});
