...
form.addEventListener('submit', async (event) => {
  ...
  if (!file) {
    setStatus('error', 'אנא צרפו הפניה רפואית לפני השליחה.');
    return;
  }

  submitBtn.disabled = true;
  form.classList.add('is-loading');
  setStatus('info', 'מעלה את ההפניה הרפואית ושולח את הבקשה לטופס 17...');
  ...
  try {
    ...
    setStatus(
      'success',
      'הבקשה נשלחה בהצלחה! במידה והכל תקין, תקבלו טופס 17.'
    );
    form.reset();
    setFile(null);
    showSuccessModal();
